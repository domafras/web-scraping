...
if __name__ == "__main__":
    ...
    while True:
        # Requisição e parsing da pagina
        page_content = fetch_page()
        product_info = parse_page(page_content)
        current_price = product_info['new_price']

        # Obtenção do maior preco historico
        max_price, max_price_timestamp = get_max_price(conn)

        # Logica de comparação de preços
        if max_price is None or current_price > max_price:
            print(f'Preço maior detectado: {current_price}')
            max_price = current_price # atualiza o maior preco
            max_price_timestamp = product_info['timestamp'] # atualiza o timestamp do maior preco
        else:
            print(f'\nO maior preço registrado foi R$ {max_price} em {max_price_timestamp}')
